document.addEventListener("DOMContentLoaded", function () {
    ...
    // Handle regular navigation (e.g., using the browser's back button)
    window.addEventListener("popstate", function (event) {
        if (event.state && event.state.articleId) {
            // If there is an articleId in the state, handle the navigation accordingly
            const destination = `/works.html#${event.state.articleId}`;
            window.location.href = destination;
        }
    });
});
